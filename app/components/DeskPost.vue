...
<!--         <div class="img" 
        ...
        <div
          class="img"
          :class="imgClass"
          v-for="(pic, i) in picture"
          :key="'pic' + i"
        >
          <img
            class="zoomable"
            :src="pic"
            @click="$emit('open-gallery', picture, i)"
            loading="lazy"
          />
        </div>
      </div>
...
const imgClass = ref("");
if (props.picture) {
  imgClass.value = props.picture.length==1?"col-1"
  :props.picture.length==2?"col-2"
  :props.picture.length==4?"col-2"
  :"col-3"
}
